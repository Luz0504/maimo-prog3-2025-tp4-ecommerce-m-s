...
const BurgerMenu = () => {
  ...
  return (
    <>
       ...
       <h2 className='text-[1.25em] font-bold border-b-2 border-dotted pb-1'>Categories</h2>
       {
        categories.filter(cat => cat.type === "1").map(cat =>
           <Link href={`/category/${cat._id}`} onClick={() => getCategoryProducts(cat._id)} key={cat._id} className='text-left list-none py-2 border-y-2 border-dotted hover:bg-blue-400 hover:cursor-pointer hover:font-bold pl-3 border-b-0 text-2xl'>&#11208; {cat.name}</Link>
        )
        }
